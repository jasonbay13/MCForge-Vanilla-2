    ...
    public class CmdShutdown : ICommand
    {
        ...
        public void Use(Player p, string[] args)
        {
            if (args.Length > 1)
            {
                ...
                {
                    alive = false;
                    Player.UniversalChat("Server shutdown has been canceled");
                    return;
                }
                if (int.TryParse(args[0], out b))
                {
                    alive = true;
                    Player.UniversalChat("Server shutdown in " + b + " seconds.");
                    for (int i = b; i > 0; i--)
                    {
                        Thread.Sleep(1000);
                        if (!alive) return;
                        Player.UniversalChat(i != 1 ? "Server shutdown in " + (i - 1) + " seconds." : "Server Shutting Down!");
                    }
                }
                else { Help(p); return; }
            }
